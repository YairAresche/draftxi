...
- [ ] **Curación de datos — top 10 naciones 1978–2026** — ver prompt en `scripts/prompt-data-curation.mjs`. Workflow: abrir chat nuevo con Claude, pegar el prompt, responde de a 1 mundial por mensaje ("siguiente" para continuar). Guardar cada respuesta en `scripts/curated/batch-[AÑO].json` y correr `node scripts/integrate-curated.mjs`. 13 mundiales × ~10 selecciones = ~130 squads a cubrir. España 2022 ya está hecho.
